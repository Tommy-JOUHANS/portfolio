...
reports/views.py — Endpoints PDF.
  ...
  POST /api/audits/{id}/generate-report/   admin → déclenche Celery, renvoie 202
...
class GenerateReportView(APIView):
    ...
    def post(self, request, audit_id):
        ...
        # 1 rapport max par demande : upsert
        report, _ = AuditReport.objects.update_or_create(
            audit_request=audit,
            defaults={
                "summary": request.data.get("summary", ""),
                "verdict": request.data.get("verdict", ""),
                "security_score": int(request.data.get("security_score", 50)),
                "grade": request.data.get("grade", "C"),
                "findings": request.data.get("findings", []),
                "pdf_path": "",         # remis à zéro → on regénère
                "generated_at": None,
            },
        )
        ...
        return Response(
            {"id": str(report.id), "status": "generating"},
...
class ReportDownloadView(APIView):
    ...
    def get(self, request, audit_id):
        ...
        # PDF pas encore généré → 202 Accepted (anti-polling abusif côté front)
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{audit.reference}.pdf"'
        )
